...
整个项目流程为：AngularJS调用MVC站点层提供的接口==>MVC站点再调用WebApi接口进行数据包装==>WebApi站点除了提供自身接口，同时还提供其他子系统接口(Rest ful)。  
整个后端业务采用[T4模板](https://github.com/sjh37/EntityFramework-Reverse-POCO-Code-First-Generator)反向生成实体类的EF模式，面向接口编程，用[Simple Injector容器](https://github.com/simpleinjector/SimpleInjector)组件解耦层，数据层则使用了[Mehdime.Entity](https://github.com/mehdime/DbContextScope)进行读写分离，通过[AutoMapper](https://github.com/AutoMapper/AutoMapper)进行model到dto的投影。  
...
# 技能清单
* 前端：Html/Css，JavaScript，Ajax，Jquery，EasyUI/Ligerui，Bootstrap，AngularJS，Echarts
* 后端：C#，Asp.Net MVC/WebApi，EF，Linq，DDD，Lambda，Dapper，Aspx，IIS，.Net Core，Go
* 其他：SSO，Consul，AutoMapper，Simple Injector，Log4Net，NLog
* 数据库：MSSQL，MySQL，SQLite
* 版本管理：Svn，Git，GitHub，GitLab
